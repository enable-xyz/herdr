...
`idle` 和 `done` 都表示可以接受输入。客户端和 CLI/API 共享服务器的已查看状态：`done` 表示尚未标记为已查看的 idle。聚焦终端窗口会在所有客户端中将其显示的标签页标记为已查看；显式 `pane focus` / `agent focus` 会标记目标，读取不会。`blocked` 表示 Herdr 识别到审批或提问界面。`unknown` 表示智能体存在但无法可靠分类,不代表工作成功。
