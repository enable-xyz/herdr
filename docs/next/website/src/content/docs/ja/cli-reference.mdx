...
`idle` と `done` はどちらも入力可能な状態です。クライアントと CLI/API はサーバーの既読状態を共有し、未確認の idle を `done` とします。ターミナルウィンドウにフォーカスすると表示中のタブが全クライアントで既読になります。明示的な `pane focus` / `agent focus` は対象を既読にしますが、読み取りでは変わりません。`blocked` は承認または質問 UI を Herdr が認識した状態です。`unknown` はエージェントが存在するものの確実に分類できない状態で、作業の成功を意味しません。
