...
Set `content_margins = true` under `[ui]` to add breathing room around terminal content without shrinking the native tab row or sidebar. On desktop, the pane surface stays centered at no more than 90 columns, with a smaller minimum margin beside a visible sidebar and a larger one when the sidebar is hidden. This keeps the shared terminal width identical between sidebar and editor-only windows when both have room for the capped surface and their margins. With a 26-column sidebar, that requires 122 host columns; a hidden-sidebar window requires 110. Matching viewport heights are also needed to avoid terminal resizes on focus changes. Pane borders and scrollbars can consume columns within the surface. Margins shrink to preserve at least 80 surface columns, disappear at 80 columns and below and in the mobile layout, and add one row above and below when the available pane surface is at least 12 rows tall. The default is `false`.
