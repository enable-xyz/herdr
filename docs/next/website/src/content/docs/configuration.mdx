...
Set `content_margins = true` under `[ui]` to add breathing room around terminal content without shrinking the native tab row or sidebar. On desktop, the pane surface is centered at 80 columns whenever the available area is wide enough, including a 124-column window with a 36-column sidebar. Narrower areas use their available width without horizontal margins. Sidebar and editor-only windows with room for the 80-column surface and matching viewport heights therefore share terminal dimensions without focus-driven resizing. Pane borders and the server's scrollbar gutter still consume columns within the surface: an ordinary single pane has 79 text columns. When there is a right margin, a single visible pane's scrollbar is drawn at the far-right window edge, with clicks, thumb dragging, and incremental updates following that location; its original gutter is left blank. Multiple visible panes retain their individual scrollbar positions. One row is added above and below when the available pane surface is at least 12 rows tall. Mobile layout is unchanged. The default is `false`.
...
Only panes with a valid native session reference from an official integration can resume; other panes restore as normal shells. A restored agent starts when its pane becomes visible in an active client (or on direct terminal attach), not when a hidden workspace or tab is loaded. See [Session state and restore](/docs/session-state/) for supported Agents and persistence behavior.
