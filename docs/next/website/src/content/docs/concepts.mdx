...
Clients and the CLI/API share the server's seen state. Focusing a terminal window marks the agents in that client's displayed tab seen, clearing their Done indicators in every connected client. Other tabs remain unseen. Both `idle` and `done` mean ready for input.
...
To open another native client at an existing workspace, run `herdr client --workspace <id>`. This focuses that workspace once after the initial snapshot; the client can navigate freely afterward. For a window that should close with its launch workspace, use `herdr client --workspace <id> --exit-on-workspace-close`. This opt-in detaches that client successfully when the original workspace closes, without switching it to another workspace; changing focus or closing only a pane does not end it. Other clients and the server keep running. Without the option, clients continue their normal behavior.
