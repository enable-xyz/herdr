...
Set `content_margins = true` under `[ui]` to add breathing room around terminal content without shrinking the native tab row or sidebar. On desktop, the pane content stays centered at no more than 120 columns, with a smaller minimum margin beside a visible sidebar and a larger one when the sidebar is hidden. Margins shrink to preserve at least 80 content columns, disappear at 80 columns and below and in the mobile layout, and add one row above and below when the available pane surface is at least 12 rows tall. The default is `false`.
