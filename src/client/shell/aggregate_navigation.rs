...
impl<'a> ClientAgentViewEntry<'a> {
    fn new(row: &AggregateAgentRow<'a>) -> Self {
        Self {
            ...
            agent: row.agent,
            // Done/Idle is the endpoint's shared unseen/seen projection; never re-acknowledge it
            // from this client's surface.
            seen: row.agent.agent_status != crate::api::schema::AgentStatus::Done,
        }
    }
}
